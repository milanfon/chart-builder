...
export class Page {
    ...
    renderParameters(paramLine, headLine) {
        ...
        for (let i = 0; i < paramCount + 1; i++) {
            ...
            for (let c = 0; c < columnCount + 1; c++) {
                if (i < 1 && c < 1) {
                    ...
                    params += `<text x="${dimensions.specs.padding + dimensions.specs["param-line"] / 2}" y="${headLine + (i+0.5) * unitHeight}" fill="#${colors.general.outline}" text-anchor="middle" align-baseline="middle" font-family="Russo One" font-size="${dimensions.specs["font-size"].default}" dominant-baseline="central">${this.props.parameters[i - 1]}</text>`;
                } else if (i > 0) {
                    const cellValue = this.props.values[c - 1].val[i - 1];
        ...
        for (let c = 1; c < columnCount + 1; c++) {
            params += `<text x="${paramLine + (c - 0.5) * valUnit}" y="${headLine + (0.5) * unitHeight}" fill="#${colors.general.background}" text-anchor="middle" align-baseline="middle" font-family="Russo One" font-size="${dimensions.specs["font-size"].default}" dominant-baseline="central">${this.props.values[c - 1].name}</text>
